...
public class BattleMap {
	...
	public void setEnemyInfo(ScannedRobotEvent e) {
		while (recordLock == true)// 锁，防止潜在的线程问题（事件驱动的模型似乎没有线程同步问题）
			;
		recordLock = true;
		...
		enemy.setX(newX);
		enemy.setY(newY);
		if (!isNew && !battle.cooperate.isTeammate(enemy.getName())) {// 基于模式匹配瞄准的敌人移动状态记录
			System.out.println(enemy.getName());
	...
	private double predictAim(RobotInfo target, double bulletSpeed) {
		...
		// 计算坦克的角度，需要根据车身的运动对炮管瞄准进行补偿，减少由于车子的运动造成误差
	...
	public void calcuBestTarget() {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (battle.cooperate.isTeammate(target.getName()))
			...
			target.setAimPrice(target.getEnergy() / 5 + target.getDistance() / 20);// 由于是团队模式，优先击杀能量低、距离近的敌人
	...
	public NextMoveInfo calcuNextGravityMove(Force outsideForce, int direction) {
		...
		while (enumeration.hasMoreElements()) {
			...
			GravityPoint point;
			if (!battle.cooperate.isTeammate(enemy.getName()))
		...
		// 计算场上固定的重力点，包括四个角落、四面墙和中央点，这些位置坦克是不会去的
