...
public class BattleMap {
	...
	public NextAimInfo calcuNextGunBearing() {
		...
		if (Math.sin(Math.toRadians(angleErrorRange)) * aimingTarget.getDistance() < 10 && battle.getGunHeat() == 0) {
			if (aimingTarget.predictDistance < 400 || aimingTarget.energy == 0) {
				nextAimInfo.setPower(400 / aimingTarget.predictDistance + 1);
	...
	private double predictAim(RobotInfo target) {
		...
		target.predictX = target.predictX
				+ Math.sin(target.getHeading() / 180 * Math.PI) * hitTime * target.getVelocity();
		target.predictY = target.predictY
				+ Math.cos(target.getHeading() / 180 * Math.PI) * hitTime * target.getVelocity();
